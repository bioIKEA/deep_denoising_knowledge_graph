...
from scipy.sparse.csgraph import csgraph_from_dense, connected_components, connected_components, minimum_spanning_tree
...
def dataSplit(labeled_ratio, train_ratio, noise_ratio, seed=1111):
    ...
    print(" train size ", _size_train, "size of test", _size_test)
    ...
    if _size_train >= len(min_span_tree):
        train_ones = []
        train_ones = np.array(min_span_tree).tolist()
        ...
        train_ones = []
        train_ones = random.Random().sample(min_span_tree.tolist(), len(min_span_tree))
...
if __name__ == '__main__':
    ...
    for train_ratio in train_ratio_list:
        res_file = "baseline_without_unlabeled_train_ratio_%s_.txt" % (str(train_ratio))
