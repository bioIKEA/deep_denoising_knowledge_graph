...
class LinkPredictionCriterion(Callback):
    ...
    def __init__(self, invoke_every, val_ones, val_zeros, max_patience, resfile):
        super().__init__(invoke_every)
        self.val_ones = val_ones
        self.val_zeros = val_zeros
        self.max_patience = max_patience
        self.res_file_name = resfile
    ...
    def invoke(self, loss, model):
        ...
        start = time.time()
        model.update_scores_matrix()
        roc_auc, avg_prec, true_y, pred_y = utils.link_prediction_performance(
        ...
        f = open(self.res_file_name, append_write)
        ...
        f.write(f"Step: {step_str}/{model.steps}"+f"Loss: {loss:.5f}"\
                +f"ROC-AUC Score: {roc_auc:.3f}"+ f"Average Precision: {avg_prec:.3f}"\
                # + f"F1 Score: {f1_score:.3f}" + f"Recall: {recall:.3f}" \
                + f"Total-Time: {int(model.total_time)}\n")
        ...
        utils.save_tofile([true_y, pred_y], self.res_file_name)
