...
class AddNoise:
    """
    REMOVE: flip 1 -> 0 only
    SWAP: flip 1 -> 0 and 0 -> 1
    """
    REMOVE = 1
    SWAP = 2
...
def train_test_split_adjacenty(total_size, test_neg_size, test_indexes):
    """
    ...
    for (x, y, val) in test_indexes:
        if val == 1:
            ones.append([x, y])
        else:
            zeros.append([x, y])
    print("test size with positive = ", len(ones), "with negative size = ", len(zeros))
    seed = 1111
    test_neg = random.Random(seed).sample(zeros,test_neg_size)
    test_pos = random.Random(seed).sample(ones, test_neg_size)
    train_pos = []
    
    # make test neg = test pos
    # test_neg = random.Random(seed).sample(test_neg, int(len(ones)*test_ratio))
    print("test pos = ", len(test_pos), test_pos[:3])

    filter_set = set()
    for (i, j) in ones:
        if [i, j] not in test_pos:
            train_pos.append((i, j))
            filter_set.add(i)
            filter_set.add(j)
    print("train pos", len(train_pos))
    filter = sorted(list(filter_set))

    for i in range(max_degree_id):
        train_pos.append((i, max_degree_id))
    for j in range(max_degree_id + 1, total_size):
        train_pos.append((max_degree_id, j))

    train_pos = np.array(train_pos)
    test_pos = np.array(test_pos)
    test_neg = np.array(test_neg)
    symmetrize = lambda x: np.row_stack((x, np.column_stack((x[:, 1], x[:, 0]))))
    train_pos = symmetrize(train_pos)
    test_pos = symmetrize(test_pos)
    test_neg = symmetrize(test_neg)

    print("train size = ", len(train_pos), train_pos[:, 0], train_pos[:, 1])
    print("test data in largest connected component size is", len(test_pos), len(test_neg))

    return np.array(filter), np.array(train_pos), np.array(test_pos), np.array(test_neg)

    # all_ones = A.nonzero()
    # print("all ones = ",A.count_nonzero())
    # train_ones = []
    # for x, y in zip(*all_ones):
    #     train_ones.append([x, y])
    #     train_ones.append([y, x])

    # for test_idx in test_nodes_index:
    #     if (test_idx, linked_node_idx) in set(map(tuple, train_ones)):
    #         continue
    #     train_ones.append([test_idx, linked_node_idx])
    #     train_ones.append([linked_node_idx, test_idx])
    # print("total size = ", total_size)
    # for idx in range(total_size):
    #     print("id  = ", idx)
    #     if idx in test_nodes_set:
    #         continue
    #     elif (linked_node_idx, idx) in set(map(tuple, train_ones)):
    #         continue
    #     else:
    #         train_ones.append([linked_node_idx, idx])
    #         train_ones.append([idx, linked_node_idx])

def synthetic_dataset_split(A, train_ratio, file_name):
    # step 1: get non zero edges of adj matrix
    A = sp.tril(A).tocsr()
    A.eliminate_zeros()

    E, N = A.nnz, A.shape[0]

    _size_train = int(E * train_ratio)
    _size_test = E - _size_train

    sampling_zeros = []

    while len(sampling_zeros) < _size_test:
        r, c = np.random.randint(0, N, 2)
        if A.todense().item(r, c) == 0 and r > c and (r, c) not in sampling_zeros:
            sampling_zeros.append((r, c))

    sampling_zeros = np.array(sampling_zeros)

    A_hold = minimum_spanning_tree(A)
    A_sample = A - A_hold
    # print("A ======= size", A_hold.nnz, A_sample.nnz, A.nnz)
    x, y = A_hold.nonzero()
    min_span_tree = np.column_stack((x, y))
    i, j = A_sample.nonzero()
    other_ones = np.column_stack((i, j))

    # print(" sampling zeros", len(sampling_zeros))
    save_tofile([min_span_tree, other_ones, sampling_zeros], file_name)

# def synthetic_dataset_split(A, labeled_ratio, file_name):
#     # step 1: get non zero edges of adj matrix
#     A = sp.tril(A).tocsr()
#     A.eliminate_zeros()
#
#     E, N = A.nnz, A.shape[0]
#
#     size_labeled = int(E * labeled_ratio)
#     train_ratio = labeled_ratio
#     _size_zeros = int(size_labeled * (1 - train_ratio))
#     sampling_zeros = []
#
#     while len(sampling_zeros) < _size_zeros:
#         r, c = np.random.randint(0, N, 2)
#         if A.todense().item(r, c) == 0 and r > c and (r, c) not in sampling_zeros:
#             sampling_zeros.append((r, c))
#
...
def train_val_test_split_adjacency(A, p_val=0.10, p_test=0.05, seed=0, neg_mul=1,
    ...
    print("E = ", A.nnz, "N = ", N, "s_train", s_train)
    idx = np.arange(N)
    ...
    if every_node:
        ...
        # print("A_sample", A_sample, "s_train", s_train, "A_hold.nnz", A_hold.nnz)
    else:
        A_sample = A
    ...
    print("s_val_ones = ", s_val_ones, "s_val_zeros = ", s_val_zeros)
    ...
    print("val ones = ", val_ones.shape, "val zeros = ", val_zeros.shape)
    ...
    if undirected:
        ...
        print("val ones = ", val_ones.shape, "val zeros = ", val_zeros.shape, len(set(map(tuple, val_zeros))))
        A = A.maximum(A.T)
...
def random_walk(graph, rwlen, n_walks=1):
    ...
    #nodes = lists
    # print("size of sample nodes = ", len(nodes))
    for w in range(n_walks):
        source_node = np.random.choice(nodes)
        # print("source node = ", source_node)
        walk.append(graph.node2vec_walk(walk_length=rwlen, start_node=source_node))

    # print("shpae of ranm walk", np.array(walk).shape)
...
class RandomWalker:
    ...
    def __init__(self, G, rw_len,filename, p=1, q=1, batch_size=128):
        is_directed = False
        self.train_G = node2vec.Graph(G, is_directed, p, q)
        # print("test nodes", self.train_G.G.nodes())
        ...
        print("preprocess trainsition probs", filename)
        if not os.path.isfile(filename):
            self.train_G.preprocess_transition_probs(filename)
        [self.train_G.alias_nodes, self.train_G.alias_edges] = load_variable(filename)
        print("finish the transition process")
        self.rw_len = rw_len
    ...
    def walk(self):
        while True:
            yield random_walk(self.train_G, self.rw_len, self.batch_size).reshape([-1, self.rw_len])
