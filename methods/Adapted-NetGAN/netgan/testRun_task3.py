...
def run_experiment(res_in_one_file, case_number, labeled_ratio, train_ratio, noise_ratio, weight_decay, walk_p=0.1, walk_q=10, mode="debug", baseline=False, seed=1111, epoch_times=3):
    ...
    while epoch_times:
        ...
        if _size_train >= len(min_span_tree):
            ...
            test_ones = random.Random(seed).sample(other_ones.tolist(), _size_test)
            ...
            if len(complement_train_ones) > 0 and  len(noise_ones) - len(complement_train_ones) > 0:
                print("len noize one",len(noise_ones), len(complement_train_ones), len(noise_zeros))
                noise_ones = random.Random().sample(noise_ones, len(noise_ones) - len(complement_train_ones))
                # assert len(complement_train_ones) + len(noise_ones) == len(noise_zeros)
            elif len(noise_ones) - len(complement_train_ones) <=0:
                noise_ones = []
        ...
        f.write("current ROC =  {0}\n current F1 =  {1}\n current Precision =  {2}\n current Recall = {3}\n accuracy = {4}\n".format(
                log_dict["ROC"], log_dict["F1"], log_dict["Precision"], log_dict["Recall"], log_dict["Accuracy"]))
        epoch_times -= 1
...
if __name__ == '__main__':
    ...
    # weight_decay_list = [0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]
    weight_decay_list = [1]
    # noise ratio ->    1:1(1/2),   1:3(3/4),   1:5(5/6),  1:7 (7/8), 1:9(9/10)
    # noise_ratio_list = [0.5,        0.75,        0.83,     0.875, 0.9]
    noise_ratio_list = [0.9]

    labeled_ratio = 0.5
    # noise_ratio = 0.5
    train_ratio = 0.5

    res_in_one_file = "extend_task_3_labeled_ratio_%s_non_baseline.txt" %(str(labeled_ratio))

    for weight_decay in weight_decay_list:
        case_number = 1
        for noise_ratio in noise_ratio_list:
            tmp_name = "with_weight_%s_noise_%s" % (str(weight_decay),str(noise_ratio))
            run_experiment(res_in_one_file, tmp_name + "_" + str(case_number), labeled_ratio, train_ratio, noise_ratio, weight_decay, walk_p=walk_p, walk_q=walk_q,
                           mode="non-debug", baseline=False, seed=seed, epoch_times=epoch_times)
            case_number += 1
