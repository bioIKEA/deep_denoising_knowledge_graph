...
def train_test_split_adjacenty(total_size, test_ratio, test_indexes):
    ...
    seed = 1111
    test_neg = random.Random(seed).sample(zeros, int(len(zeros) * test_ratio))
    train_neg = []
    for (i, j) in zeros:
        if (i, j) not in test_neg:
            train_neg.append((i, j))
    test_pos = random.Random(seed).sample(ones, int(len(ones) * test_ratio))
    train_pos = []
