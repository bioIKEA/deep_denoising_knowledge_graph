...
def run_experiment(res_in_one_file, case_number, labeled_ratio, train_ratio, noise_ratio, weight_decay, walk_p=0.1, walk_q=10, baseline=False, seed=1111, epoch_times=3):
    """
    ...
    while epoch_times:
        ...
        if _size_train >= len(min_span_tree):
            ...
            if noise_ratio > 0:
                removed_size = int(len(unlabeled_ones)*noise_ratio)
                noise_ones = random.Random().sample(noise_ones, len(noise_ones) - removed_size)
...
if __name__ == '__main__':
    ...
    res_in_one_file = "task_0_labeled_%s_noise_%s_train_%s_non-baseline.txt" %(str(labeled_ratio), str(noise_ratio), str(train_ratio))
    tmp_name = "with_weight_decay_%s" % str(weight_decay)
    run_experiment(res_in_one_file, tmp_name + "_" + str(case_number), labeled_ratio, train_ratio, noise_ratio,
                   weight_decay, walk_p=walk_p, walk_q=walk_q, baseline=False, seed=seed, epoch_times=3)
    ...
    #                    baseline=False, seed=seed, epoch_times=3)
    #     case_number += 1
