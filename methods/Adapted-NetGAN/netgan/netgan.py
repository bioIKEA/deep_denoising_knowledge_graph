...
class NetGAN:
    ...
    def __init__(self, N, rw_len, boolean_mask,walk_generator, generator_layers=[40], discriminator_layers=[30],
        ...
        print("real inputs", self.real_inputs)
        ...
        # true_index = tf.constant(filter_idx, dtype=tf.int32)
        # print("true index", true_index)
        self.boolean_mask = tf.constant(boolean_mask, dtype=tf.float32)
        self.fake_inputs = tf.multiply(self.fake_inputs, self.boolean_mask)
        self.real_inputs = tf.multiply(self.real_inputs, self.boolean_mask)

        # self.fake_inputs = tf.boolean_mask(self.fake_inputs, boolean_mask, axis=2)
        # self.real_inputs = tf.boolean_mask(self.real_inputs, boolean_mask, axis=2)

        # print("fake inputs", self.fake_inputs)
        # print("real inputs", self.real_inputs)
    ...
    def generator_recurrent(self, n_samples, reuse=None, z=None, gumbel=True, legacy=False):
        ...
        with tf.variable_scope('Generator') as scope:
            ...
            print("generated output is", outputs)
        return outputs
    ...
    def train_then_test(self, A_orig, test_ones, test_zeros, max_iters=50000, eval_transitions=15e4,
                        transitions_per_iter=1500, save_directory="../snapshots", \
        ...
        test_ones: np.array, shape (n_val, 2)
                  The indices of the hold-out set of validation edges
        test_zeros: np.array, shape (n_val, 2)
        ...
        log_dict: dict
                  ...
                  * The test performances (ROC and AP)
        ...
        # testing labels
        actual_labels_val = np.append(np.ones(len(test_ones)), np.zeros(len(test_zeros)))
        ...
        print("sample count", sample_many_count)
        ...
        print("n_eval walks",n_eval_walks)
        print("n_eval_iters", n_eval_iters)
        ...
        print("start testing with iteration = ", n_eval_iters)
        smpls = []
        ...
        edge_scores = np.append(gr[tuple(test_ones.T)].A1, gr[tuple(test_zeros.T)].A1)
        # print("edge scores", edge_scores)


        """
        save the confusion matrix for result analyze
                        ground true, ground false
        predict true    ture positive, false postiive,
        predict false   false negative, True negative
        """
        fold = "../data/covid_19/test_dataset/"
        True_positive_filename = "T_pos_test.tsv"
        True_negative_filename = "T_neg_test.tsv"
        False_positive_filename = "F_pos_test.tsv"
        False_negative_filename = "F_neg_test.tsv"
        ...
        list_dict_filename = "../data/covid_19/node_list_dict.pkl"
        idx_to_text_filename = "../data/covid_19/idx_to_text.pkl"
        node_list = []
        node_dict = dict()
        dict_list = []
        ...
        if os.path.isfile(idx_to_text_filename):
            [dict_list] = load_variable(idx_to_text_filename)

        jump_steps = 1

        start = 0
        with open(fold + True_positive_filename, 'wt') as out_file:
            tsv_writer = csv.writer(out_file, delimiter='\t')
            for x, y in test_ones:
                if edge_scores[start] > jump_steps:
                    true_positive += 1
                    tsv_writer.writerow([edge_scores[start], set(getkeyText(dict_list, node_list[x])), set(
                        getkeyText(dict_list, node_list[y]))])
                start += 1
        start = 0
        with open(fold + False_negative_filename, 'wt') as out_file:
            tsv_writer = csv.writer(out_file, delimiter='\t')
            for x, y in test_ones:
                if edge_scores[start] <= jump_steps:
                    false_negative += 1
        ...
        start = len(test_ones)
        with open(fold + False_positive_filename, 'wt') as out_file:
            tsv_writer = csv.writer(out_file, delimiter='\t')
            for x, y in test_zeros:
                if edge_scores[start] > jump_steps:
                    false_positive += 1
                    tsv_writer.writerow([edge_scores[start], set(getkeyText(dict_list, node_list[x])), set(
                        getkeyText(dict_list, node_list[y]))])
        ...
        start = len(test_ones)
        with open(fold + True_negative_filename, 'wt') as out_file:
            tsv_writer = csv.writer(out_file, delimiter='\t')
            for x, y in test_zeros:
                if edge_scores[start] <= jump_steps:
                    true_negative += 1
        ...
        val_performances.append((roc_auc_score(actual_labels_val, edge_scores),
                                 ...
                                 f1_score(actual_labels_val, (edge_scores > jump_steps).astype(int))
                                 ))

        print("test dataset ground truth with ones = ", len(test_ones)//2, len(test_zeros)//2)

        print("True positive", true_positive//2)
        print("True negative", true_negative//2)
        print("False positive", false_positive//2)
        print("False negative", false_negative//2)
        ...
        print("**** Iter {:<6} Val ROC {:.3f}, AP: {:.3f}, F1 :{:.3f}, EO {:.3f} ****".format(_it,
                                                                                  val_performances[-1][0],
                                                                                  val_performances[-1][1],
                                                                                  val_performances[-1][2],
                                                                                  edge_overlap / A_orig.sum()))

        return {"actual_label": actual_labels_val, "edge_score": edge_scores, "ROC": val_performances[-1][0], "F1": val_performances[-1][2], "Precision": val_performances[-1][1], \
